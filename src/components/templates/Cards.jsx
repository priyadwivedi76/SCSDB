        ...
        {data.map((d,index)=>{
            return(
                <Link to={`/${d.media_type || title}/details/${d.id}`} key={index} className="relative w-[25vh]">
                    <img className="h-[45vh] object-cover shadow-[8px_17px_38px_2px_rgba(0,0,0,0.5)]" src={d.poster_path || d.backdrop_path || d.profile_path ?`https://image.tmdb.org/t/p/original/${d.poster_path || d.backdrop_path || d.profile_path}`:noImage} alt="" />
